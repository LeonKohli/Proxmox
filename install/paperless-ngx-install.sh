...
msg_info "Installing Apache Tika for Office Document Support"
$STD apt-get install -y openjdk-11-jre-headless
$STD wget https://archive.apache.org/dist/tika/tika-server-2.6.0.jar -O /opt/paperless/tika-server.jar
sed -i -e 's|#PAPERLESS_TIKA_ENABLED=false|PAPERLESS_TIKA_ENABLED=true|' /opt/paperless/paperless.conf
sed -i -e 's|#PAPERLESS_TIKA_ENDPOINT=http://localhost:9998|PAPERLESS_TIKA_ENDPOINT=http://localhost:9998|' /opt/paperless/paperless.conf
...
cat <<EOF >/etc/systemd/system/paperless-tika.service
[Unit]
Description=Apache Tika Server for Paperless
After=network.target
Wants=network.target

[Service]
WorkingDirectory=/opt/paperless
ExecStart=/usr/bin/java -jar /opt/paperless/tika-server.jar
...
$STD systemctl enable --now paperless-consumer paperless-webserver paperless-scheduler paperless-task-queue paperless-tika
